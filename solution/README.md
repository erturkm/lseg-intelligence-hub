...
The exported Power Platform solution (`managed` and `unmanaged` `.zip`) is published on the
[Releases page](../../releases) rather than committed here, to keep the repository small and
to keep each export tied to a specific version tag.
...
1. Download the `unmanaged` zip (for a dev environment you intend to customise) or the
   `managed` zip (for a downstream/test environment).
